...
class EventBusSupervisor {
  ...
  static void emit(Event event) =>
    _instance._eventBus.emit(event);
  
  /// Закрыть шину данных
  /// ВНИМАНИЕ, ЭТО НЕОБРАТИМО
  static Future<void> close() =>
    _instance._eventBus.close();
  
  /// Поток событий  
  static Stream<Event> get events =>
    _instance._eventBus.events;

  /// Поток ошибок
  static Stream<EventBusException> get errors =>
    _instance._eventBus.errors;
  ...
  void _onError(EventBusException error) =>
    _exceptionCallbacks.forEach((EventBusExceptionCallback callback) => callback(error));
