...
class EventBus {
  ...
  Stream<Transition<PrevEvent, NextEvent>>
      whereTransition<PrevEvent extends Event, NextEvent extends Event>(
          ...
          events.transform(
              WhereTransitionTransformer<Event, PrevEvent, NextEvent>(
  ...
  EventBus._internalSingleton() : _state = _EventBusState() {
...
class _EventBusState implements Sink<Event> {
  ...
  _EventBusState() {
    ...
    errors = _controller.stream.transform<EventBusException>(
        StreamTransformer<Event, EventBusException>.fromHandlers(
      handleData: (Event nextEvent, EventSink<EventBusException> _) =>
          lastEvent = nextEvent,
      handleError: (Object error, StackTrace stackTrace,
              EventSink<EventBusException> sink) =>
